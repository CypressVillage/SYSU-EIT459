...
% www.nt.tuwien.ac.at
% =====================================================================    
% This script simulates the bit error ratio over a time-variant multipath
% channel (doubly-selective channel) for different multicarrier modulation
% techniques:
...
Simulation_SNR_OFDM_dB            = [-5:5:35];                          % SNR for OFDM in dB. The average transmit power of all methods is the same! However, the SNR might be different due to filtering (in FOFDM and UFMC) or because a different bandwidth is used (different subcarrier spacing or different number of subcarriers).
Simulation_MonteCarloRepetitions  = 500;                                % Number of Monte Carlo Repetitions for which we take the average                  
Simulation_PlotSignalPowerTheory  = true;                               % If TRUE, plot also the theoretical expected transmit power over time
...
Channel_DopplerModel         = 'Jakes';                                 % Which Doppler model: 'Jakes', 'Uniform', 'Discrete-Jakes', 'Discrete-Uniform'. For "Discrete-", we assume a discrete Doppler spectrum to improve the simulation time. This only works accuratly if the number of samples and the velocity is sufficiently large                                       
Channel_CarrierFrequency     = 2.5e9;                                   % Carrier Frequency (Hz)

% General modulation paramters
QAM_ModulationOrder          = 64;                                      % QAM sigal constellation order: 4,16,64,256,,...
...
FBMC_PrototypeFilter         = 'Hermite-OQAM';                          % Prototype filter (Hermite, PHYDYAS, RRC) and OQAM or QAM. The data rate of QAM is reduced by a factor of two compared to OQAM, but robustness in doubly-selective channels is inceased. Note that our simulation is no longer valid for QAM because we transmit real valued symbols but could transmit complex valued symbols with QAM
...
WOLA_CyclicPrefixLength      = 0;                                       % Length of the cyclic prefix (s)
...
FOFDM_CyclicPrefixLength      = 0;                                      % Length of the cyclic prefix (s)
...
UFMC_CyclicPrefixLength      = 0;                                       % Length of the cyclic prefix (s). If zero padding is used, this length reprents the zero guard length instead of the CP length
...
ChannelModel = Channel.FastFading(...
    ...
    false,...                                                           % channel time correlation
    'none',...                                                          % sptial correlation for MIMO channels
    0,...                                                               % transmit side spatial correlation coefficient
    0,...                                                               % receive side spatial correlation coefficient
    ...
    0,...                                                               % TWDP K parameter
    0 ...                                                               % TWDP delta parameter
    );

% PAM and QAM Object
PAM = Modulation.SignalConstellation(sqrt(QAM_ModulationOrder),'PAM',0);
QAM = Modulation.SignalConstellation(QAM_ModulationOrder,'QAM',0);
...
for i_SNR = 1:length(Simulation_SNR_OFDM_dB)
    ...
    DetectedBitStream_OneTapEqualizer_FBMC  = PAM.Symbol2Bit(real(y_OneTapEqualizer_FBMC(:)));
...
fprintf('===========================================================================================\n');
fprintf('=================================Basic Settings============================================\n');
fprintf('               |(complex)TF-Spacing|   Bandwidth  |     Time    |      BitRate \n');
...
fprintf('OFDM (with CP) | %17.2f mJ |%8.2f ms  |%14.2f W  |%14.2f dB | \n', sum(Ps_OFDM)*dt/1e-3   , OFDM.PHY.TimeSpacing*OFDM.Nr.MCSymbols/1e-3   , sum(Ps_OFDM)*dt/(OFDM.PHY.TimeSpacing*OFDM.Nr.MCSymbols)    , 10*log10( mean(mean(OFDM.GetSymbolNoisePower(1))) / mean(mean(OFDM.GetSymbolNoisePower(1)))    ));
fprintf('FBMC-OQAM      | %17.2f mJ |%8.2f ms  |%14.2f W  |%14.2f dB | \n', sum(Ps_FBMC)*dt/1e-3   , FBMC.PHY.TimeSpacing*FBMC.Nr.MCSymbols/1e-3   , sum(Ps_FBMC)*dt/(FBMC.PHY.TimeSpacing*FBMC.Nr.MCSymbols)    , 10*log10( mean(mean(OFDM.GetSymbolNoisePower(1))) / mean(mean(FBMC.GetSymbolNoisePower(1)))*2  ));
fprintf('WOLA           | %17.2f mJ |%8.2f ms  |%14.2f W  |%14.2f dB | \n', sum(Ps_WOLA)*dt/1e-3   , WOLA.PHY.TimeSpacing*WOLA.Nr.MCSymbols/1e-3   , sum(Ps_WOLA)*dt/(WOLA.PHY.TimeSpacing*WOLA.Nr.MCSymbols)    , 10*log10( mean(mean(OFDM.GetSymbolNoisePower(1))) / mean(mean(WOLA.GetSymbolNoisePower(1)))    ));
fprintf('FOFDM          | %17.2f mJ |%8.2f ms  |%14.2f W  |%14.2f dB | \n', sum(Ps_FOFDM)*dt/1e-3  , FOFDM.PHY.TimeSpacing*FOFDM.Nr.MCSymbols/1e-3 , sum(Ps_FOFDM)*dt/(FOFDM.PHY.TimeSpacing*FOFDM.Nr.MCSymbols) , 10*log10( mean(mean(OFDM.GetSymbolNoisePower(1))) / mean(mean(FOFDM.GetSymbolNoisePower(1)))   ));
fprintf('UFMC           | %17.2f mJ |%8.2f ms  |%14.2f W  |%14.2f dB | \n', sum(Ps_UFMC)*dt/1e-3   , UFMC.PHY.TimeSpacing*UFMC.Nr.MCSymbols/1e-3   , sum(Ps_UFMC)*dt/(UFMC.PHY.TimeSpacing*UFMC.Nr.MCSymbols)    , 10*log10( mean(mean(OFDM.GetSymbolNoisePower(1))) / mean(mean(UFMC.GetSymbolNoisePower(1)))    ));
fprintf('===========================================================================================\n');
fprintf('===========================================================================================\n');
